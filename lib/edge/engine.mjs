...
export class EdgeEngine extends EventEmitter {
  ...
  ingestOdds(rec) {
    ...
    for (const { side, prob, idx } of this._fairProbs(rec)) {
      ...
      // LIQUIDITY PROXY (edge #2, task3_edge_research_sweep §5): quote count in the last
      // 60s for THIS market. The classifier gates follow-vs-fade on it — edge_lab showed
      // THICK lines (many quotes) carry (β>0, underreaction) while THIN lines mean-revert
      // (β<0, noise), so a thin book is where a stale line gets picked off.
      const quoteDensity = this._densityWithin(m.history, now, 60_000);
      ...
      if (pastSteam != null && !recentEvent) {
        ...
        if (Math.abs(delta) >= this.cfg.steamThreshold) {
          steamFired = !!this._fire("steam", marketKey, m.meta, {
            ...
            fairProb: prob,
            preEventProb: pastSteam, // pre-move fair prob (surprise conditioning in the classifier)
            quoteDensity, // liquidity gate (edge #2): thick=carry/follow, thin=noise/revert
        ...
        if (atEvent != null) {
          ...
          if (Math.abs(swing) >= this.cfg.overreactionThreshold) {
            overFired = !!this._fire("overreaction", marketKey, m.meta, {
              ...
              fairProb: prob,
              preEventProb: atEvent, // fair prob at the goal/red-card instant → surprise proxy
              quoteDensity, // liquidity gate (edge #2): thin books revert harder → fade support
  ...
  fairProbForMarket(meta) {
    const f = this.markFrameForMarket(meta);
    return f ? f.prob : null;
  }

  // Latest REAL ingested frame for an edge's market: the fair prob AND the frame
  // timestamp it came from. The runner marks/settles on this so the exit leg is
  // tied to an actual observed TxLINE frame (verifiable), and uses `ts` to refuse
  // settling until a frame strictly AFTER entry exists.
  markFrameForMarket(meta) {
    ...
    return { prob: last.prob, ts: last.ts };
  }
  ...
  // quote count within the last `windowMs` — the edge #2 liquidity proxy.
  _densityWithin(history, now, windowMs) {
    const from = now - windowMs;
    let n = 0;
    for (let i = history.length - 1; i >= 0 && history[i].ts >= from; i--) n++;
    return n;
  }
