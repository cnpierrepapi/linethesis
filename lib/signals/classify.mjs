...
export function classifyEdge(edge, ctx = {}) {
  ...
  const { liquidity, driftRegime } = liquidityRegime(quoteDensity);
  ...
  return {
    ...
    revertLikely,
    pickoffRisk,
    liquidity,            // edge #2: 'thin'|'thick'|null — book regime from quote density
    driftRegime,          // edge #2: 'carry' (thick→drift/follow) | 'revert' (thin→noise) | null
    lateMatch,            // edge #6: in the closing ~20min where drift amplifies
    ...
    note: driftRegime || lateMatch
      ? `${edge.note}${driftRegime ? ` · ${liquidity} book → ${driftRegime === "carry" ? "move carries (edge #2)" : "likely noise/revert (edge #2)"}` : ""}${lateMatch ? " · late-match drift amplifies (edge #6)" : ""}`
      : edge.note,
  };
}
